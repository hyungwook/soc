...
int main(void)
{
	...
	float firstx = 0, firsty = 0, secondx = 0, secondy = 0;
	...
	while (b_loop)
	{
		...
		while (1)
		{
			...
			*******************************************************
			...
			for (i = 0; i < 120; i++)
			{
				...
				for (j = 0; j < 180; j++){
					...
					*(lcd + i * 180 + j) = *(fpga_videodata + i * 180 + j);
					...
					else if (((int)*(blue + i * 180 + j) > 10) && ((int)*(hue_joon + i * 180 + j) > 180) && ((int)*(hue_joon + i * 180 + j) < 240))
						*(xxx + i * 180 + j) = 6;//파랑을표시
			...
			for (i = 0; i < 5; i++)
			{
				firsty += *(out_i + cnt0 / 2 - 5 + i);
				firstx += *(out_j + cnt0 / 2 - 5 + i);
				secondy += *(out_i + cnt0 / 2 + 5 - i);
				secondx += *(out_j + cnt0 / 2 + 5 - i);
			}
			first_y = firsty / 10;
			first_x = firstx / 10;
			second_y = secondy / 10;
			...
			if (stage == 1)
			{
				...
				stage1:
					...
					printf("sum2 : %d\n", sum2);
					/*
					if (sum2>sum1+300)//오른쪽에 파란픽셀이 많을 때
					{
					printf("right\n");
					...
					else if (sum2<sum1-300)//왼쪽에 파란픽셀이 많을 때
					{
					printf("left\n");
