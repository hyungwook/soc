	...
	while (b_loop)
	{
		...
		GOUP:
			...
			*/
			...
			if (stage == 1)
			{
				int aver = 0; int fir = 0;
				cnt1 = 0;
				sum1 = 0;
				sum2 = 0;
				printf("walk\n");
				Send_Command(0x15, 0xea);//두걸음
			...
			stage1:
				...
				for (i = 40; i < 80; i++)
				{
					...
					if (cnt1 > 40 && cnt1 < 125)// 가로줄 하나에 cnt1가 40 이상이면 cnt1값과 sum2값을 다른 변수에 합하면서 저장
					{
						aver = aver + cnt1;
						sum3 = sum3 + sum2;
						cnt1 = 0;
				...
				if (sum3 >= 100 && sum3 < 200)
				{
					...
					Send_Command(0x06, 0xf9);
					DelayLoop(50000000);
					Send_Command(0x15, 0xea);//앞으로 한걸음
					Send_Command(0x15, 0xea);
					DelayLoop(50000000);
					Send_Command(0x15, 0xea);
					Send_Command(0x15, 0xea);
			...
			if (stage == 4)
			{
				...
				for (j = 0; j < 180; j++)
				{
					for (i = 120; i > 0; i--)
					{
						...
						*(lcd + i * 180 + j) = 0x07e0;
						...
						/*if ((*(xxx + 180 * i + j) + *(xxx + 180 * i + j - 1) == 12)
							|| (*(xxx + 180 * i + j) + *(xxx + 180 * (i - 1) + j) == 12)
							|| (*(xxx + 180 * i + j) + *(xxx + 180 * (i - 1) + j - 1) == 12))//파검일 때
							{
							center_p += i;
							center_q += j;
							hole++;
				...
				printf("center i : %d  center j : %d\ncenter p : %d  center q : %d", center_i, center_j, center_p, center_q);
